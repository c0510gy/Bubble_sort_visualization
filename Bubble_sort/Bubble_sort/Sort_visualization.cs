...
class Sort_visual
{
    ...
    private void color_transform(ref int f, ref Bitmap frame, ref Graphics g, ref Box[] boxes, int N, int j1, int j2, Color clr, string str = "")
    {
        int c_r = boxes[j1].clr.R, c_r_ = clr.R;
        int c_g = boxes[j1].clr.G, c_g_ = clr.G;
        int c_b = boxes[j1].clr.B, c_b_ = clr.B;

        int c_r2 = boxes[j2].clr.R, c_r2_ = clr.R;
        int c_g2 = boxes[j2].clr.G, c_g2_ = clr.G;
        int c_b2 = boxes[j2].clr.B, c_b2_ = clr.B;
        ...
        for (int i = 0; i <= d; i++)
        {
            ...
            int r2_ = (int)(c_r2 + (double)(c_r2_ - c_r2) * ((double)i / (double)d));
            int g2_ = (int)(c_g2 + (double)(c_g2_ - c_g2) * ((double)i / (double)d));
            int b2_ = (int)(c_b2 + (double)(c_b2_ - c_b2) * ((double)i / (double)d));

            boxes[j1].clr = Color.FromArgb(255, r_, g_, b_);
            boxes[j2].clr = Color.FromArgb(255, r2_, g2_, b2_);
    ...
    public void Bubble_Sort(ref int[] arr)
    {
        int f = 0;
        ...
        for (int j = 0; j < N; j++)
        {
            for (int i = 0; i < N - j - 1; i++)
            {
                ...
                color_transform(ref f, ref frame, ref g, ref boxes, N, i + 1, Color.OrangeRed, (i + 1) + " 인덱스와 비교");
                if (arr[i] > arr[i + 1])
                {
                    swap_transform(ref f, ref frame, ref g, ref boxes, N, i, i + 1, i + " 인덱스와 " + (i + 1) + " 인덱스 swap");
                    swap_box(ref boxes, i, i + 1);
                    Swap(ref arr, i, i + 1);
                }
                color_transform(ref f, ref frame, ref g, ref boxes, N, i, i + 1, Color.SkyBlue, i + " 인덱스와 " + (i + 1) + " 인덱스 정렬 완료");
            }
            color_transform(ref f, ref frame, ref g, ref boxes, N, N - j - 1, Color.Orange, (N - j - 1) + " 인덱스 자동으로 정렬됨");
        }
